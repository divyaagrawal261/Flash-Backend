...
export const loginPlayer = async (req, res) => {
  ...
  try {
    ...
    } else res.status(403).json("Invalid Credentials");
  } catch (err) {
    console.log(err);
    res.status(500).json(err.message);
  }
...
export const getLoggedInUser = async (req, res) => {
  try {
    ...
    if (!_id) throw new Error("User logged Out");

    const loggedInUser = await player.find({ _id });

    if (!loggedInUser) throw new Error("User doesn't exist");
...
export const bookSlot = async (req, res) => {
  ...
  try {
    ...
    if (!existingSlot) res.status(404).json("Slot does not exist");
    ...
    if (!existingSlot.players) existingSlot.players = [];
...
export const allSlots = async (req, res) => {
    ...
    try {
      ...
      if (!bookedSlots) return res.status(404).json("Couldn't display slots");
  
      const slotDetails = [];
  
      await Promise.all(bookedSlots.bookings.map(async (slotId) => {
        const _id = slotId.toString();
        const slotDetail = await slots
          .findById(_id)
          .populate("playgroundId ownerId");
  
        const playgroundName = slotDetail.playgroundId.name;
        const playgroundLocation = slotDetail.playgroundId.location;
        const slotTime = slotDetail.time;
        const slotDate = slotDetail.date;
        const slotSize = slotDetail.slotSize;
        const players = slotDetail.players.length;
